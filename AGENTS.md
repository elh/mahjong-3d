...
- Do not gate native screen saver visibility or playback on CSS animation/transition completion. Drive fades and completion from the native frame timestamps because inactive WKWebView scheduling can leave an opaque overlay stuck over a correctly rendered scene.
- When bumping a release version, update `package.json`, `macos/Mahjong3D.xcodeproj/project.pbxproj` (`MARKETING_VERSION` and `CURRENT_PROJECT_VERSION` for app and extension targets), and the legacy saver plist at `macos/Mahjong3D/Info.plist`. The modern app and extension Info.plists read those Xcode build settings.
